...
describe('/users', () => {
    ...
    it('should seed start data', async () => {
        ...
        const auth1 = await request(app).post('/auth/login').set('user-agent', 'Chrome').send(correctInputModelAuth)
        accessToken1 = auth1.body.accessToken
        cookie1 = auth1.header['set-cookie']
        const auth2 = await request(app).post('/auth/login').set('user-agent', 'Chrome').send(correctInputModelAuth2)
        accessToken2 = auth2.body.accessToken
        cookie2 = auth2.header['set-cookie']
        const auth3 = await request(app).post('/auth/login').set('user-agent', 'Chrome').send(correctInputModelAuth3)
        accessToken3 = auth3.body.accessToken
        const auth4 = await request(app).post('/auth/login').set('user-agent', 'Chrome').send(correctInputModelAuth4)
